// Verifica l'arma dalla prospettiva top-down REALE di gioco (quella che vede il giocatore),
// zoomata sul personaggio per giudicare la precisione dell'impugnatura.
...
await sleep(2500);
// camera top-down come in gioco ma zoomata sul personaggio (stesso angolo ~55°)
await page.evaluate(() => {
  const c = window.__CONFIG.camera;
  c.offsetY = 7.5; c.offsetZ = 5.5; c.lerp = 30; c.aimPull = 0;
});
...
await shot('pistol', 'tools/td_pistol.png', 760, 350);
await shot('shotgun', 'tools/td_shotgun.png', 760, 350);
